...
/**
 * Error thrown when the Pro Analytics API returns an error.
 */
export class ProAnalyticsApiError<T = unknown> extends Error {
  /**
   * Error thrown when the Pro Analytics API returns an error.
   * @param response The response from the Pro Analytics OrderBook API.
 ...
 * @see {@link Backoff configuration: }
 */
export const DEFAULT_BACKOFF_OPTIONS: BackoffOptions = {
  numOfAttempts: 10,
  maxDelay: Infinity,
  jitter: "none",
  retry: (error: Error | ProAnalyticsApiError) => {
    if (error instanceof ProAnalyticsApiError) {
...
/**
 * The default rate limiter options for Pro Analtics API.
 *
 * **CAUTION**: The Pro Analytics API is limited to 5 requests per second per IP.
 */
...
export async function request<T>(
    ...
    return backOff<T>(async () => {
      ...
      return Promise.reject(new ProAnalyticsApiError(response, responseBody))
    }, backoffOpts)
  }